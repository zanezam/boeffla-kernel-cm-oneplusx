...
MKBOOTIMG_CMDLINE="androidboot.hardware=qcom user_debug=23 msm_rtb.filter=0x3b7 ehci-hcd.park=3 androidboot.bootdevice=msm_sdcc.1"
MKBOOTIMG_BASE="0x00000000"
MKBOOTIMG_PAGESIZE="2048"
MKBOOTIMG_RAMDISK_OFFSET="01000000"
...
step3_compile()
{
	...
	if [ "y" == "$COMPILE_DTB" ]; then
		...
		if [ -f $BUILD_PATH/$OUTPUT_FOLDER/arch/arm/boot/dt.img ]; then
			rm $BUILD_PATH/$OUTPUT_FOLDER/arch/arm/boot/dt.img
		fi
